 #JustPy app containing a HighCharts graph that represents aggregated average ratings
 # by day of the week.
...
weekday_aver = data.groupby(['Weekday']).count()
...
def app():
    ...
    hc.options.xAxis.categories = list(weekday_aver.index) # creation of a new catgory part in the xAxis 

    # to make x = [3,4], y = [4,5 ] it into [[3,4],[6,7]]  we use the zip command 
    hc.options.series[0].data = list(weekday_aver['Rating'])
